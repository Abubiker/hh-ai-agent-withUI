...
Три правила для файла (в отличие от UI-панели, которая живёт только на
время сеанса):
  ...
  3. Длинные строки (сопроводительные письма, ответы на тест работодателя —
     свободный текст из резюме пользователя) обрезаются: файлу для
     диагностики хватает начала строки, а пересылка резюме целиком
     постороннему — лишний риск, которого можно было избежать.
"""
import logging
...
# Сопроводительные письма и ответы на вопросы теста работодателя — свободный
# текст, собранный из профиля пользователя (settings.resume_summary), а не
# секрет с известным значением, который можно вычистить точным совпадением.
# Файл лога по докстрингу модуля можно переслать разработчику для разбора
# зависания — ему не нужен текст письма целиком, достаточно первых символов,
# чтобы понять, что там вообще было (пусто? галиматья? не на русском?).
# Полный текст пользователь по-прежнему видит в окне приложения и в
# уведомлении — это усечение касается только файла.
MAX_LOGGED_LINE = 300


def _truncate(line: str) -> str:
    if len(line) <= MAX_LOGGED_LINE:
        return line
    return line[:MAX_LOGGED_LINE] + f"… [обрезано для файла, всего {len(line)} симв.]"
...
def log(line: str, level: str = "info", origin_hint: str | None = None):
    ...
    try:
        ...
        _logger.log(_LEVELS.get(level, logging.INFO), _truncate(_redact(line)),
                    extra={"origin": o})
