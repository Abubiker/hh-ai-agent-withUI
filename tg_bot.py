...
async def start_bot():
    ...
    while True:
        # get_bot() внутри цикла, а не один раз до него: иначе смена токена
        # в Настройках, пока polling уже упал и ждёт реконнекта, никогда не
        # подхватится — луп продолжал бы стучаться со СТАРЫМ Bot-объектом
        # до перезапуска всего приложения.
        bot = get_bot()
        if not bot:
